...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (socket && socket.on) {
      ...
      socket.on('ticket:created', (data) => {
        ...
        addLiveActivity('ticket_created', data.ticket);
      });
      ...
      socket.on('ticket:resolved', (data) => {
        ...
        addLiveActivity('ticket_resolved', data.ticket);
      });

      socket.on('ticket:assigned', (data) => {
        toast.success(`Ticket assigned: ${data.ticket.subject}`);
        addLiveActivity('ticket_assigned', data.ticket);
      });
      ...
      socket.on('user:created', (data) => {
        if (user?.role === 'admin') {
          toast.success(`New user created: ${data.user.name}`);
          addLiveActivity('user_created', data.user);
        }
      });
      ...
      // Category management events (Admin only)
      socket.on('category:created', (data) => {
        if (user?.role === 'admin') {
          toast.success(`New category created: ${data.category.name}`);
          addLiveActivity('category_created', data.category);
        }
      });
      ...
      socket.on('agent:workload_updated', (data) => {
        if (user?.role === 'agent' || user?.role === 'admin') {
          addLiveActivity('agent_workload_updated', data);
        }
      });
      ...
      socket.on('performance:metrics_update', (data) => {
        addLiveActivity('performance_update', data);
      });
      ...
      return () => {
        if (socket && socket.off) {
          ...
          socket.off('agent:workload_updated');
  ...
  // Add live activity to the feed
  const addLiveActivity = (type, data) => {
    const activity = {
      id: Date.now(),
      type,
      data,
  ...
  // User management data (Admin only)
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Recent Tickets with Real-time Updates */}
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              <div className="flex items-center justify-between mb-4">
              ...
              {ticketsLoading ? (
                ...
                <div className="space-y-4">
                  {recentTickets.tickets.map((ticket) => (
                    <div key={ticket._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                ...
                {Object.entries(systemStatus).map(([service, status]) => (
                  <div key={service} className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      {service === 'realtime' && <Wifi className="w-4 h-4 text-gray-600" />}
                      {service === 'ai' && <Brain className="w-4 h-4 text-gray-600" />}
                      {service === 'storage' && <HardDrive className="w-4 h-4 text-gray-600" />}
                      {service === 'network' && <Network className="w-4 h-4 text-gray-600" />}
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium">{userStats.activeUsers || 0}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">Agents</span>
                    <span className="text-sm font-medium">{userStats.agents || 0}</span>
                  </div>
                  <div className="flex justify-between items-center">
